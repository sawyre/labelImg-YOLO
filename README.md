...
#### Ручная разметка (наличие cv2, TensotFlow, Keras необязательно)
1) В файле data/predefined_classes.txt определите список классов для разметки.
2) Откройте директорию с картинками и определите место сохранения аннотаций.
3) Выделите необходимые объекты, сохраните аннотацию.
4) Перейдите к следующей картинке

![Снимок экрана при ручной разметке](https://github.com/sawyre/labelImg-YOLO/blob/master/demo/demo01.png)

#### Одноклассовая разметка. 
1) В файле data/predefined_classes.txt определите список классов для разметки.
2) Откройте директорию с картинками и определите место сохранения аннотаций.
3) В панели View нажмите Single Class mode, создайте рамку и определите класс объекта, при выделении следующих классов им автоматически будет присвоен этот класс
 
![Вкладка “View”](https://github.com/sawyre/labelImg-YOLO/blob/master/demo/demo02.png)

Альтернативно: установите галочку «Use default label» в окне  Box Labels и впишите класс поумолчанию, далее, для выделения объекта этого класса достаточно лишь создать рамку.

![Окно “Box Labels”](https://github.com/sawyre/labelImg-YOLO/blob/master/demo/demo03.png)

Данный способ разметки позволяет быстро разметить выборки, содержащие один класс, или несколько, если применить его последовательно несколько раз.
 
#### Полуавтоматическая разметка.

![Снимок экрана при использовании авторазметки](https://github.com/sawyre/labelImg-YOLO/blob/master/demo/demo1.jpg)

1) Установите папку, содержащую изображения подлежащие разметке(Autolabeling->Set yolo images path), или воспользуйтесь автоматически созданной директорией Documents/labelImg/Images.
2) Выберите папку, в которую буду сохраняться аннотации(Autolabeling->Set yolo results path), или воспользуйтесь автоматически созданной директорией Documents/labelImg/Results
3) Укажите путь до файла конфигурации YOLO для конкретной задачи(Autolabeling->Set yolo config path), или воспользуйтесь одним из файлов в папке wheight-configs.
4) Укажите путь до файла весов YOLO для конкретной задачи (Autolabeling->Set yolo wheights path), или воспользуйтесь одним из файлов в папке wheight-configs.
5) Если требуется, измените пороговое значение для нейросетевого детектора (Autolabeling->Set yolo threshold)
6) Запустите авторазметку указанной директории (Autolabeling ->Autolabel Dir). Авторазметка большого количества изображений может занять некоторое время, прогресс разметки будет отображаться в окне «autolabeling progress».
7) После завершения авторазметки откройте размеченную директорию и начните проверку. Доступные для разметки классы определятся из файла конфигурации YOLO. Используясь горячие клавиши, перемещайтесь по выделенным объектам и редактируйте классы, выделенные неверно.
